...
function get () {
    ...
    .then(function(data){
        parseData(data.businesses, states[i]);
        limit=data.total;
        console.log("\n\n--------------------------New Request---------------------\n\n", 'State: ' + location, '\nCategory: ' + mains[m],'\nOffset: ' + offset + "/" + limit);
        offset+=20;
        console.log(i,"i","states[i]",states[i]);
        if(offset>limit){
          console.log("finish category", mains[m]);
          ...
          if (m === mains.length){ //if out of categories then stop getting.
              console.log(
                ...
                m=0;  //start from beginning category
          }
            if(i===states.length){
    ...
    .catch(function(err){
        console.error(err,"error catch in get request", mains[m], 'mains[m]wwww');//reached 1000 pieces of data
...
function parseData (data,state){
    ...
    for(var i = 0;i<data.length;i++){
        if(data[i].location.coordinate && data[i].location.postal_code===state){ //check if it has coordinates
            var obj = {
    ...
    if (objarray.length > 1){
        ...
        submitOne(objarray[0]);
    }
}

function submitBulk (data) {
    Business.collection.insert(data,{},function(err,res){
      console.log("response: Added", res.result.n);
        console.log('\nInsert Error: ', err);
    });
}
function submitOne (data) {
    Business.create(data,{},function(err,res){
      console.log("response: added one");
        console.log('\nInsert Error: ', err);
    });
}
